...
class Websocket:
    ...
    def read_frame(self, max_size=None):
        ...
        if length == 126:  # Magic number, length header is 2 bytes
            length, = struct.unpack('>H', self.read_all(2))
        elif length == 127:  # Magic number, length header is 8 bytes
            length, = struct.unpack('>Q', self.read_all(8))

        if mask:  # Mask is 4 bytes
            mask_bits = self.read_all(4)
    ...
    def read_all(self,length):
        print("RX: reading %d bytes" % (length))
        data = b''
        while len(data) < length:
            partial = self.sock.read(length - len(data))
            print("RX: read %d bytes" % (len(partial)))
            data += partial
    ...
    def write_all(self,data):
        ...
        while offset < len(data):
            offset += self.sock.write(data[offset:])
    ...
    def write_frame(self, opcode, data=b''):
        ...
        print("opcode=%02x data=%s" % (opcode, data))
        ...
        if length < 126:  # 126 is magic value to use 2-byte length header
            byte2 |= length
            self.write_all(struct.pack('>BB', byte1, byte2))

        elif length < (1 << 16):  # Length fits in 2-bytes
            byte2 |= 126  # Magic code
            self.write_all(struct.pack('>BBH', byte1, byte2, length))

        elif length < (1 << 64):
            byte2 |= 127  # Magic code
            self.write_all(struct.pack('>BBQ', byte1, byte2, length))
